// ==========================================
// 1. FIREBASE AUTH & WEBRTC SIGNALING
// ==========================================
...
if (auth) {
    auth.onAuthStateChanged((user) => {
        if (user) {
            currentUsername = (user.displayName || user.email.split('@')[0]).toUpperCase();
            saveLocalProfile();
        }
    });
}
...
window.addEventListener('keydown', (e) => {
    ...
    switch(uiState) {
        ...
        case 'ALIAS':
            if (e.key === 'g' || e.key === 'G') {
                loginWithGoogle();
            } else if (e.key === 'Enter' && btnContinue) { 
                playMenuSelectSound(); 
                btnContinue.click(); 
            }
            break;
...
// ==========================================
// 6. WEBRTC MULTIPLAYER SIGNALING VIA FIRESTORE
// ==========================================
async function createRoom() {
    if (!db) { alert("Firebase Database is not initialized."); return; }
...
async function joinRoom() {
    ...
    if (inputId.length < 5) { alert("PLEASE ENTER A VALID 5-CHARACTER ROOM CODE."); return; }
    
    isHost = false;
    const roomRef = db.collection('rooms').doc(inputId);
    ...
    await roomRef.update({ answer: { type: answer.type, sdp: answer.sdp }, guestName: currentUsername });
}
...
function update(time) {
    ...
    if (activeWord) {
        activeWord.y += (currentFallSpeed * 0.75) * dt;
